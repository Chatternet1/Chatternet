...
app.post('/api/signup', (req, res) => {
  ...
  const user = {
    ...
    privacy: {
      ...
      dmAudience: 'everyone', online: false, tags: true,
      search: true, activity: false, location: false
    },
    settings: {
      darkMode: false, compact: false, highContrast: false,
      reduceMotion: false, fontSize: 'medium', theme: 'blue', language: 'en',
...
/* ------------ events CRUD (used by Events page) ------------ */
const ensureEventShape = (ev) => ({
  ...
  rsvp: ev.rsvp && typeof ev.rsvp === 'object'
    ? {
        Going: Array.isArray(ev.rsvp.Going) ? ev.rsvp.Going : [],
        Maybe: Array.isArray(ev.rsvp.Maybe) ? ev.rsvp.Maybe : [],
        NotGoing: Array.isArray(ev.rsvp.NotGoing) ? ev.rsvp.NotGoing : []
      }
...
app.get('/api/events', (_req, res) => {
  data.events = (data.events || []).map(ensureEventShape);
...
app.get('/api/events/:id', (req, res) => {
  ...
  res.json(ensureEventShape(ev));
});

app.post('/api/events', (req, res) => {
  const incoming = ensureEventShape({ ...req.body, id: Date.now().toString(), createdAt: nowISO() });
  (data.events ||= []).unshift(incoming);
  save();
  res.json(incoming);
});
...
app.put('/api/events/:id', (req, res) => {
  const id = req.params.id;
  const i = (data.events || []).findIndex(e => Strin
